...
- Form-data → `--form-string 'k=v'` per field; the informational `multipart/form-data` Content-Type is omitted because curl generates its own boundary — exactly mirroring the executor's `removeContentTypeHeader` before send. Only text fields can occur (the Postman parser drops file-typed fields), so no `-F 'k=@file'` path is needed.
