...
- **[Risk] `ENV_PICKER_VERTICAL_OVERHEAD` miscounted** → The chrome row count (border + padding + title + spacer + footer) must be exact or the panel will overflow by 1-2 rows on edge-case terminal heights. **Mitigation:** Count conservatively (7 rows) and test at 12, 16, 20, 24, and 40-row terminals. The `Math.max(1, ...)` floor prevents negative visible counts.
