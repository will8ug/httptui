...
- Add cursor-chase scrolling: as the highlight moves past the visible window, the window shifts to keep the highlight in view (reusing the existing `getVisibleRequestOffset` helper).
...
- **`src/core/reducer.ts`** — `ENTER_ENV_SELECT` and `CANCEL_ENV_SELECT` reset `envSelectScrollOffset`; `MOVE_ENV_SELECTION` applies `getVisibleRequestOffset`; new `JUMP_ENV_SELECTION` case; `createInitialState` initializes the new field.
