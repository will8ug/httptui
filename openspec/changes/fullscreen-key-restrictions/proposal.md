...
- Normal-mode key handling becomes a default-deny whitelist while a panel is maximized; every non-whitelisted key is a silent no-op (matching the existing `Tab`/`d` no-op precedent):
  - **Always live**: navigation (`j`/`k`/`g`/`G`/`0`/`$`/`h`/`l` and arrow keys), `f` (exit fullscreen), `Escape` (cancel in-flight request first, then clear active search results while the response panel is maximized, else exit fullscreen), `?` (help overlay), `Ctrl+C` (exit — the interrupt key always terminates, per the terminal convention already codified in the unsaved-changes spec).
...
- `Escape` priority in a maximized response panel reverses: with active search state, the first `Escape` clears the search and stays fullscreen, and only a following `Escape` exits fullscreen (previously fullscreen-exit won and search clearing happened after exit). In maximized requests/details, exit-first priority is unchanged.
...
- `response-search`: gate `/`, `n`, `N` to response-maximized fullscreen; keep `q` dismissal working regardless of which panel is maximized while removing the quit fallback in fullscreen; Escape dismissal takes priority over exiting fullscreen while the response panel is maximized.
- `navigation`: the Escape priority chain gains a rung — while the response panel is maximized with active search state, clearing search results precedes exiting fullscreen.
