...
- **Remove** the `raw` parameter from `formatResponseBody` — always pretty-print JSON responses
...
- `tui`: Remove the `r` keyboard shortcut and `rawMode` state from the TUI interface spec. Response body rendering always uses the default (pretty-printed JSON) behavior.
