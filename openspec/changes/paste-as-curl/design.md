...
`{ type: 'APPEND_REQUEST'; request: ParsedRequest }` appends the request, sets `selectedIndex` to the new last index, clamps `requestScrollOffset` to bring it into view, resets `requestHorizontalOffset` and details offsets, and clears search state — exactly what `SELECT_REQUEST` does today (which leaves `response` untouched; mirroring keeps paste consistent with every other selection change). The request carries `isDirty: true`. The parser emits `lineNumber: 0` as a "no file origin" placeholder — the domain is 1-based, so `0` can never be mistaken for a real file line (and in-place save re-parses the file rather than trusting in-memory values). But `RequestList` builds composite React keys as `${lineNumber}-${method}-${url}`, so pasting the same command twice with a literal `0` would collide keys. The `APPEND_REQUEST` reducer — the only place that sees the full request list — therefore replaces the placeholder with `max(existing lineNumbers) + 1`. That value is never interpreted as a file position; it exists solely to keep keys unique.
