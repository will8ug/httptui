...
The request details panel SHALL appear inside the right panel column, above the response view. When visible, the right column SHALL be a vertical flex container with the request details on top and the response view below. The panel height SHALL be content-driven (adapting to number of headers/body lines) with a maximum of approximately 10 rows.
...
#### Scenario: Long request body is truncated
- **WHEN** the resolved request body exceeds the panel's maximum visible height
- **THEN** the panel SHALL truncate the body display with an ellipsis indicator
...
The request details panel SHALL use visual styling consistent with the existing application: a bordered box with the method displayed in an HTTP-method-appropriate color, the URL in a distinct color, headers in gray, and a horizontal separator between sections.
