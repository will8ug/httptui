...
The system SHALL extract the request body based on body mode. For `raw` mode, the raw string SHALL be used directly. For `urlencoded` mode, the key-value array SHALL be converted to a `key=val&key2=val2` string and a `Content-Type: application/x-www-form-urlencoded` header SHALL be injected. For unsupported modes (`formdata` with files, `file`, `graphql`), the parser SHALL log a warning and set body to `undefined`.
...
#### Scenario: Warn on unsupported body mode (file upload)
- **WHEN** a Postman request has body mode `formdata` containing a file field
