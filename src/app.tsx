...
import React, { type Dispatch, useEffect, useReducer, useRef } from 'react';
import { useApp, useInput, useStdout, type Key, type SuspendTerminal } from 'ink';
...
import type { Action, AppProps, AppState, CertEntry, ExecutorConfig, FileVariable, ParsedRequest, ResponseData } from './core/types';
...
async function sendSelectedRequest({ state, executorConfig, abortControllerRef, dispatch }: {
  state: AppState;
...
async function performEditorHandoff({ filePath, editor, suspend, dispatch }: {
  filePath: string;
...
async function copySelectedAsCurl({ request, variables, baseDir, certificates, insecure, clipboardRunner, dispatch }: {
...
async function pasteFromClipboard({ clipboardReadRunner, dispatch }: {
...
function handleHelpInput({ input, key, dispatch }: {
  input: string;
  key: Key;
  dispatch: Dispatch<Action>;
}): void {
...
function handleFileLoadInput({ state, input, key, executorConfig, dispatch }: {
  state: AppState;
  input: string;
  key: Key;
  executorConfig: ExecutorConfig;
...
function handleSearchInput({ state, columns, effectiveResponseHeight, effectiveDetailMaxContent, input, key, dispatch }: {
  state: AppState;
  columns: number;
...
function handleEnvSelectInput({ state, input, key, dispatch }: {
  state: AppState;
  input: string;
  key: Key;
  dispatch: Dispatch<Action>;
...
function handleSaveInput({ state, input, key, dispatch }: {
  state: AppState;
  input: string;
  key: Key;
  dispatch: Dispatch<Action>;
...
function handleEditInput({ state, selectedRequest, editorVisibleHeight, editorContentWidth, input, key, dispatch }: {
  state: AppState;
...
function handleConfirmDiscardInput({ state, exit, suspend, input, key, dispatch }: {
  state: AppState;
  exit: () => void;
  suspend: SuspendTerminal;
  input: string;
...
function handleConfirmInPlaceSaveInput({ state, input, key, dispatch }: {
  state: AppState;
  input: string;
  key: Key;
  dispatch: Dispatch<Action>;
